        ...
        <div id="light" class="white_content">
            ...
            <form class="form-horizontal col-xs-12 id" enctype="multipart/form-data" role="form">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="hidden" class="span11" id="id" value="<?php echo ($order["id"]); ?>"/>
                    ...
                    <div class="col-sm-9">
                        <select id="status">
                            <option value="<?php echo ($order["status"]); ?>">
                                <?php switch($order["status"]): case "0": ?>未处理订单<?php break;?>
                                    <?php case "1": ?>未处理订单<?php break;?>
                                    <?php case "2": ?>已成交订单<?php break;?>
                                    <?php case "3": ?>退单申请<?php break;?>
                                    <?php case "4": ?>已退单<?php break;?>
                                    <?php default: endswitch;?></option>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <span>用户名称：<?php echo ($order["wechat_name"]); ?></span><br>
                        <span>用户号码：<?php echo ($order["wechat_phone"]); ?></span><br>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1">商品信息 : </label>
                    <div class="col-sm-9">
                        <span>商品名称：<?php echo ($order["name"]); ?></span><br>
                        <span>商品单价：<?php echo ($order["price"]); ?></span><br>
                        <span>商品数量：<?php echo ($order["num"]); ?></span><br>
                        <span>商品总价：<?php echo ($order["price_sum"]); ?></span><br>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1">收货信息 : </label>
                    <div class="col-sm-9">
                        <span>收货人：<?php echo ($order["address_name"]); ?></span><br>
                        <span>收货电话：<?php echo ($order["address_phone"]); ?></span><br>
                        <span>收货地址：<?php echo ($order["address"]); ?></span><br>
                        <span>收货留言：<?php echo ($order["msg"]); ?></span><br>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1">快递单号 : </label>
                    <div class="col-sm-9">
                        <input type="text" class="span11" id="express" value="<?php echo ($order["express"]); ?>"/>
                    </div>
                </div>
...
<script>
    ...
    $("#save").click(function () {
        if (confirm('提交数据吗？')) {
            ...
            $.post("<?php echo U('home/order/do_edit');?>", {
                'id' : id,
                'express' : express,
                'status' : status,
            }, function (data) {
