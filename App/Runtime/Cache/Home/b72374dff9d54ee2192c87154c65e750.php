...
<div>
    ...
    <?php if(is_array($product_classify)): $i = 0; $__LIST__ = $product_classify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$classifys): $mod = ($i % 2 );++$i;?><a class="btn" href="/home/product/index?product_classify_id=<?php echo ($classifys["id"]); ?>"><?php echo ($classifys["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
    <a class="btn" href="<?php echo U('Product/product_add');?>">添加商品</a>
    <form action="/home/product/index" method="post">
        <input type="text" name="name" placeholder="请输入商品名称或编号">
        <input type="submit" value="查询商品">
    </form>
</div>
<div>共有<span class="ft20 c_red"><?php echo ($count); ?></span>商品</div>
<div class="widget-box">
    ...
    <div class="widget-content nopadding">
        <table class="table table-bordered data-table" style="font-size: 12px;">
            ...
            <tbody>
            <?php if(is_array($product)): $i = 0; $__LIST__ = $product;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$products): $mod = ($i % 2 );++$i;?><tr class="gradeX father">
                    <td><?php echo ($products["id"]); ?></td>
                    <td><?php echo ($products["name"]); ?></td>
                    <td><img height="50px;" src="/<?php echo ($products["pic"]); ?>"></td>
                    <td><?php echo ($products["price"]); ?> </td>
                    <td><?php echo ($products["unit"]); ?></td>
                    <td><?php echo ($products["num"]); ?></td>
                    <td><?php echo ($products["status"]); ?></td>
                    <td><?php echo ($products["desc"]); ?></td>
                    <td><?php echo (date('Y-m-d H:i:s',$products["time"])); ?></td>
                    <td>
                        <a id="<?php echo ($products["id"]); ?>" class="chstatus" href=""> 更改状态</a> |
<!--                        <a > 查看详情</a> |-->
                        <a class="icon-edit" href="/home/product/product_edit?id=<?php echo ($products["id"]); ?>"> 编辑商品</a> |
                        <a id="<?php echo ($products["id"]); ?>" class="icon-remove remove" href=""> 删除商品</a>
                    </td>
...
<script>
    $(".remove").click(function () {
        if (confirm('确定删除这条资源吗？')) {
            ...
            $.post('<?php echo U("Product/product_remove");?>',data, function (data) {
                if (data.code) {
                    alert(data.message);
