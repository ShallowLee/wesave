...
<div>
    ...
    <div class="btn-group ">
        ...
        <ul class="dropdown-menu fff">
            ...
            <li><a href="<?php echo U('news/index/status/1');?>" class="btn">上线状态</a></li>
            <li><a href="<?php echo U('news/index/status/0');?>" class="btn">下线状态</a></li>
            <!--<?php endforeach; endif; else: echo "" ;endif; ?>-->
        </ul>
    </div>
    <a class="btn" href="<?php echo U('news/add');?>">添加头条</a>
</div>
...
<div class="widget-box">
    ...
    <div class="widget-content nopadding">
        <table class="table table-bordered data-table" style="font-size: 12px;">
            ...
            <tbody>
            <?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newss): $mod = ($i % 2 );++$i;?><tr class="gradeX father">
                    <td><?php echo ($newss["id"]); ?></td>
                    <td><img height="50px;" src="/<?php echo ($newss["pic"]); ?>"></td>
                    <td><?php echo ($newss["title"]); ?></td>
                    <td><?php echo ($newss["desc"]); ?> </td>
                    <td><?php echo ($newss["status"]); ?></td>
                    <td><?php echo (date('Y-m-d H:i:s',$newss["time"])); ?></td>
                    <td>
                        <a id="<?php echo ($newss["id"]); ?>" class="chstatus" href=""> 更改状态</a> |
                        <!--                        <a > 查看详情</a> |-->
                        <a class="icon-edit" href="/home/news/edit?id=<?php echo ($newss["id"]); ?>"> 编辑头条</a> |
                        <a id="<?php echo ($newss["id"]); ?>" class="icon-remove remove" href=""> 删除头条</a>
                    </td>
...
<script>
    $(".remove").click(function () {
        if (confirm('确定删除这条资源吗？')) {
            ...
            console.log(data);
            $.post('<?php echo U("news/news_remove");?>',data, function (data) {
                    if (data.code) {
                        alert(data.message);
    ...
    $(".chstatus").click(function () {
        ...
        console.log(data);
        $.post('<?php echo U("news/chstatus");?>',data, function (data) {
